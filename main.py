...
from apache_beam import window, Map
...
def run():
    ...
    with beam.Pipeline(options=options) as p:
        ...
        #1. Write raw orders to orders_stream
        ...
        (
            ...
            | 'SumRevenueAndCount' >> beam.CombinePerKey(
                 beam.combiners.TupleCombineFn(sum, sum)
            )
            | 'FormatAggregates' >> beam.Map(lambda kv: {
                'product_name': kv[0],
                'values': {'total_revenue': kv[1][0], 'order_count': kv[1][1]},
            })
            #| 'GroupAndAggregate' >> beam.CombinePerKey(lambda records: {
                #'total_revenue': sum(r['price'] * r['quantity'] for r in records),
